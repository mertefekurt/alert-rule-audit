...
from alert_rule_audit import __version__
from alert_rule_audit.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from alert_rule_audit.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='alert-rule-audit',
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='alert rule YAML, JSON, or text export',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"alert-rule-audit: error: {exc}", file=sys.stderr)
        return 1
